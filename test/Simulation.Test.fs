...
let nashMixedTests = 
    ...
    testList "Test game modes: nashMixedStrategyEquilibriumGameFromPayoffParameters" (
        [
           {
...
let randomChoiceTests = 
    let info: GameInformation = TestData.GameInfo.getDefaultTestInfo()
    testList "Test game modes: randomChoiceGame" (
        [
            {
...
let keepSameStrategyTests = 
    ...
    testList "Test game modes: keepSameStrategy" (
        [
            {
...
let highestEuTests = 
    ...
    let testDescription = sprintf "Test game modes: highestEuOnDifferentColorGameUsingAllEncounters and highestEuOnDifferentColorGame \n\t - PayoffMatrix for all tests is: %A" ((FromRewardAndCost (10.0m, 20.0m)).ToMatrix())
...
let tests = testList "Simulation tests" [
    nashMixedTests
    randomChoiceTests
